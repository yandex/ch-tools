...
class S3ObjectLocalMetaData:
    ...
    def from_string(cls, value: str) -> Self:
        ...
        if not matches:
            raise ValueError(f'Incorrect metadata about the objects count and total size. Line: `{lines[idx]}`')
        ...
        for i in range(object_count):
            ...
            if not matches:
                raise ValueError(f'Incorrect metadata about object size and name. Line: `{lines[idx]}`')
            objects.append(S3ObjectLocalInfo(key=matches[2], size=int(matches[1])))
            idx += 1

        matches = re.match(r'^\d+$', lines[idx])
        if not matches:
            raise ValueError(f'Incorrect metadata about refcounter. Line: `{lines[idx]}`')
        refcounter = int(lines[idx])
        idx += 1

        matches = re.match('^[01]$', lines[idx])
        if not matches:
            raise ValueError(f'Incorrect metadata about readonly flag. Line: `{lines[idx]}`')
        read_only = bool(int(matches[0]))

        return cls(version=version, total_size=total_size, objects=objects, ref_counter=refcounter, read_only=read_only)
