...
@when("we stop clickhouse on {node:w}")
def step_stop_clickhouse(context: ContextT, node: str) -> None:
    container = get_container(context, node)
    result = container.exec_run(
        ["bash", "-c", "supervisorctl stop clickhouse-server"], user="root"
    )
    assert_that(result.exit_code, equal_to(0))


@when("we start clickhouse on {node:w}")
def step_start_clickhouse(context: ContextT, node: str) -> None:
    container = get_container(context, node)
    result = container.exec_run(
        ["bash", "-c", "supervisorctl start clickhouse-server"], user="root"
    )
    assert_that(result.exit_code, equal_to(0))
